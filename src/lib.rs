...
pub mod interact {
    ...
    pub async fn get_schueler(id: &i32, db: &SqlitePool) -> Result<schema::SimpleSchueler, i32> {
        if !check_schueler_id(&id) { return Err(400);}

        let query_response =
            sqlx::query_as!(model::SimpleSchueler,
            r#"
        ...
        match query_response {
            Ok(schueler) => {
                ...
                return Ok(schema::SimpleSchueler{id, first_name, last_name})
            }
            Err(_e) => {
                return Err(500)
            }
    ...
    pub async fn get_dosb_task_for_schueler(id: i32, db: &SqlitePool) -> Result<Vec<schema::PflichtKategorie>, i32> {
        if !check_schueler_id(&id) { return Err(400);}
        let query_response =
    ...
    pub async fn get_bjs_task_for_schueler(id: i32, db: &SqlitePool) -> Result<Vec<schema::PflichtKategorie>, i32> {
        if !check_schueler_id(&id) { return Err(400);}
        let query_response =
            sqlx::query_as!(model::PflichtKategorie, r#"SELECT DISTINCT kategorien.id as id, (versuch.kategorieId IS NOT NULL) AS done, kategorien.kateGroupId as group_id FROM kategorien
    ...
    pub async fn get_all_versuch_for_kat (id: i32, kat_id: i32, db: &SqlitePool) -> Result<Vec<schema::NormVersuch>, i32> {
    ...
    "#, id, kat_id).fetch_all(db).await;
        ...
        Ok(futures::future::join_all(versuche.into_iter().map(|v| calc_norm_versuch(v, &db))).await)
    }

    pub async fn get_top_versuch_by_kat(id: i32, kat_id: i32, db: &SqlitePool) -> Result<schema::NormVersuch, i32> {
        ...
        let all_versuche = futures::future::join_all(all_versuche_model.into_iter().map(|v| calc_norm_versuch(v, db))).await;
    ...
    pub async fn get_top_versuch_in_bjs(id: i32, db: &SqlitePool) -> Result<Vec<schema::NormVersuch>, i32> {
        let kat_list_result = sqlx::query_as!(model::KatId, r#"
    ...
    pub async fn get_top_versuch_in_dosb(id: i32, db: &SqlitePool) -> Result<Vec<schema::NormVersuch>, i32> {
        let kat_list_result = sqlx::query_as!(model::KatId, r#"
    ...
    pub async fn get_all_kat (db : &SqlitePool) -> Vec<schema::SimpleKategorie> {
        let query_result = sqlx::query_as!(model::SimpleKategorie, r#"
        SELECT id, name FROM kategorien
        "#).fetch_all(db).await.unwrap();

        return query_result.into_iter().map(|k| {schema::SimpleKategorie{id: k.id.unwrap() as i32, name: k.name.unwrap()}}).collect();
    }
    ...
    pub async fn calc_points(versuch: schema::SimpleVersuch, db: &SqlitePool) -> i32 {
        ...
        let kat_result = sqlx::query_as!(model::FormelKategorie, r#"
            ...
            "#, versuch.kategorie_id, versuch.schueler_id).fetch_one(db).await;
    ...
    async fn calc_norm_versuch(v: model::NormVersuch, db: &SqlitePool) -> schema::NormVersuch {
        schema::NormVersuch{
            ...
            punkte: calc_points(schema::SimpleVersuch{schueler_id: v.schueler_id.unwrap() as i32, wert: v.wert.unwrap() as f32, kategorie_id: v.kategorie_id.unwrap() as i32}, db).await.into(),
    ...
    fn kategorie_model2schema(m: model::Kategorie) -> schema::Kategorie {
        schema::Kategorie{
            ...
            name: m.name.unwrap(),
            lauf: m.lauf.unwrap(),
            einheit: m.einheit.unwrap().chars().next().expect("no Unit was given"),
...
mod tests {
    ...
    async fn get_schueler() {
        ...
        let result_schueler = schema::SimpleSchueler {id: 1234, first_name: String::from("Franz2"),last_name: String::from("Peterson")};
    ...
    async fn get_dosb_schueler() {
        let db = migrate_example_db().await;
        let test_result = interact::get_dosb_task_for_schueler(1234, &db).await.unwrap();
        assert_eq!(r#"[{"done":false,"group_id":1,"id":3},{"done":false,"group_id":2,"id":6},{"done":false,"group_id":2,"id":7},{"done":false,"group_id":3,"id":10},{"done":false,"group_id":4,"id":4}]"# ,format!("{}", serde_json::json!(test_result)));
    }
    ...
    async fn calc_points() {
        ...
        // 50m Lauf 8.4 -> 324 (w)
        assert_eq!(interact::calc_points(schema::SimpleVersuch{schueler_id: 1234, wert: 8.4, kategorie_id: 1}, &db).await, 324);

        // 800m Lauf 180 -> 329 (m)
        assert_eq!(interact::calc_points(schema::SimpleVersuch{schueler_id: 3809, wert: 180.0, kategorie_id: 4}, &db).await, 329);

        // 200g Ballwurf 27.5 -> 266 (m)
        assert_eq!(interact::calc_points(schema::SimpleVersuch{schueler_id: 3809, wert: 27.5, kategorie_id: 10}, &db).await, 266);

        // kat out of range
        assert_eq!(interact::calc_points(schema::SimpleVersuch{schueler_id: 3809, wert: 27.5, kategorie_id: 123}, &db).await, -404);

        // schuelerId out of range
        assert_eq!(interact::calc_points(schema::SimpleVersuch{schueler_id: 309, wert: 27.5, kategorie_id: 3}, &db).await, -404);
        assert_eq!(interact::calc_points(schema::SimpleVersuch{schueler_id: 35409, wert: 27.5, kategorie_id: 3}, &db).await, -404);

        // points lower than 0 or higher than 900
        assert_eq!(interact::calc_points(schema::SimpleVersuch{schueler_id: 3809, wert: 3.0, kategorie_id: 3}, &db).await, -406);
        assert_eq!(interact::calc_points(schema::SimpleVersuch{schueler_id: 3809, wert: 3.0, kategorie_id: 10}, &db).await, 0);
    ...
    async fn top_versuch_by_kat() {
        let db = migrate_example_db().await;
        let v1 = schema::SimpleVersuch{schueler_id: 1234, wert: 15.0, kategorie_id: 10};
        let current_top = schema::SimpleVersuch{schueler_id: 1234, wert: 15.0, kategorie_id: 10};
        let _ = interact::add_versuch(v1, "ABCA".to_string(), &db).await;

        // only one try
        let test_top = interact::get_top_versuch_by_kat(1234, 10, &db).await.unwrap();
        ...
        let current_top = schema::SimpleVersuch{schueler_id: 1234, wert: 17.0, kategorie_id: 10};
        let _ = interact::add_versuch(schema::SimpleVersuch{schueler_id: 1234, wert: 17.0, kategorie_id: 10}, "AWDF".to_string(), &db).await;
        ...
        let _ = interact::add_versuch(schema::SimpleVersuch{schueler_id: 1234, wert: 14.0, kategorie_id: 10}, "AWDF".to_string(), &db).await;
